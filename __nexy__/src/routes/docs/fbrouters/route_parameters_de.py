...
def Route_parameters_de(caller: Any = None, children: str = '') -> str:
    Slot = caller if (locals().get('caller') and callable(caller)) else (lambda: children if locals().get('children') else '')
    trans = __trans
    t = __trans
    pathname = usePathname()
    
    context = {"Link": Link, "usePathname": usePathname, "pathname": pathname, 'Slot': Slot, 'trans': __trans, '__locale': __current_locale.get()}
    rendered = str(__Template().render("__nexy__/src/routes/docs/fbrouters/route_parameters_de.md", context))
